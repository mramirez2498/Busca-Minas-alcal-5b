...
def click_derec(j, i, event):
    texto = tabla[j][i].cget("text")
    if texto:
        tabla[j][i].config(text="")
    else:
        tabla[j][i].config(text = "P")
